<template>
    <div v-if="!logged_in" class="text-base-content">
        <input type="text" class="input input-bordered inline-block input-sm me-3" @keyup.enter="do_login"
               placeholder="Login" v-model="login">
        <input type="password" class="input input-bordered inline-block input-sm me-3" @keyup.enter="do_login"
    ...
    <div v-else>
        <span class="me-3">Logged in as <code>{{ user_login }}</code></span>
