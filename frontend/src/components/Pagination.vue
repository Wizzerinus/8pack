<template>
    <div class="join">
        <button v-for="index in total_pages" class="join-item btn"
                :class="{'btn-active': index - 1 === current_page}"
                @click="current_page = index - 1">{{ index }}</button>
    </div>
...
export default {
    ...
    setup(props) {
        ...
        const loadPage = (set_total_pages = false) => {
            get(`${props.url}?page_num=${current_page.value}`).then(e => e.json()).then(e => {
                items.value = e.data
                total_objects.value = e.total_objects
                if (set_total_pages)
                    total_pages.value = e.data.length > 0 ? Math.ceil(e.total_objects / e.data.length) : 1
            })
        }
        loadPage(true)
