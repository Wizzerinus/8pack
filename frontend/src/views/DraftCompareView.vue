<template>
    ...
    <div class="mb-3">
        <RouterLink :to="`/drafts/${draft_id}`" class="btn btn-primary me-4">Play this draft</RouterLink>
        <RouterLink :to="`/drafts/${draft_id}/results`" class="btn btn-accent">See all results</RouterLink>
    </div>
...
export default {
    ...
    setup(props) {
        ...
        const og_run = computed(() => {
            for (const i of plays.value)
                if (i.is_og) return i
        })
        const compared_run = computed(() => {
            for (const i of plays.value)
                if (String(i.id) === props.run_id) return i
        })
        ...
        const get_border = (card, idx) => {
            ...
            if (we_picked && they_picked)
                return "picked-by-both"
            else if (we_picked)
                return "picked-by-us"
            else if (they_picked)
                return "picked-by-og"
