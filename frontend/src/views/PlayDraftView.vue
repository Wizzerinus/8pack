<template>
    ...
    <div v-else class="grid grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        <div class="col-span-2 lg:col-span-3 xl:col-span-4 mr-4">
            ...
            <div v-else>
                <button class="btn btn-primary block" v-if="logged_in" @click="save">
                    Save results
                </button>
                <span v-else class="block">Log in or register to save this draft!</span>
                <button class="btn block mt-4" @click="reset_and_view">
        ...
        <div>
            ...
            <div class="relative">
                <div
                    v-for="(card, idx) in current_picks"
                    class="absolute left-0 max-w-48"
                    :style="{ top: `${idx * 40}px` }"
                >
...
export default {
    ...
    setup(props) {
        ...
        const save = () => {
            auth_post(`drafts/${props.draft_id}/save`, {
                picks: current_picks.value.map((card) => card.id),
            })
